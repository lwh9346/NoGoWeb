...
//GoValidPositionResult 是获取可落子位置的返回结果，应传入一个指针给该函数以便该函数将数据写回
type GoValidPositionResult struct {
	numS int    //己方可落子数
	numR int    //对方可落子数
	resS []bool //己方可落子位置，1可下0不可下
	resR []bool //对方可落子位置，1可下0不可下
}

//GoGetValidPosition 获取可落子位置
func GoGetValidPosition(board []int) GoValidPositionResult {
	var arr [81]C.schar
	for i := 0; i < 81; i++ {
		arr[i] = C.schar(board[i])
	}
	var cres C.struct_ValidPositionResult
	C.GetValidPosition(&arr[0], &cres)
	cress := cres.resS
	cresr := cres.resR
	cnumS := cres.numS
	cnumR := cres.numR
	var res GoValidPositionResult
	res.resR = make([]bool, 0, 81)
	res.resS = make([]bool, 0, 81)
	for i := 0; i < 81; i++ {
		res.resS = append(res.resS, int(cress[i]) == 1)
		res.resR = append(res.resR, int(cresr[i]) == 1)
	}
	res.numR = int(cnumR)
	res.numS = int(cnumS)
	return res
